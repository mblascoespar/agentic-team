...
| Date | Change |
|------|--------|
| 2026-04-30 | **Engine fix: `get_work_context` PRD creation bug.** When creating a new PRD from an approved Brief, `handle_get_work_context(slug, stage="prd")` attempted to resolve topology (needed to find upstream stage) before the PRD existed — circular dependency. Fixed by special-casing `stage == "prd"`: upstream is always "brief" without topology lookup. Allows Product Owner skill to call `get_work_context` on first entry without requiring archetype argument; archetype is extracted from existing artifact if present. Impact: Product Owner skill can now intelligently route based on artifact state (creation, refinement, approval) without prompting for redundant input. |
| 2026-04-30 | Reversed model stage order for `system_integration + process_system` hybrid topology: `model_workflow` now precedes `model_system`. Workflow behavioral design must be approved before system integration contracts are modeled. Test assertion in `test_topology_layered_includes_both_models` updated to match. |
| 2026-04-14 | New archetype `system_evolution`: fifth standalone archetype for brownfield initiatives. Topology `brief → prd → model_evolution → design → tech_stack`. Engine: `_VALID_COMBINATIONS`, `_DAG_TOPOLOGIES`, `_BASE_SCHEMAS_BY_STAGE`, `_MODEL_TYPE_TO_STAGE`, `prd.input.json` enum updated. Schema `model-evolution.json` created (current_state, frozen_surface, change_surface, migration_path, regression_risk mandatory; open_questions optional). `/model-evolution` agent written. Tests added: `test_system_evolution_topology`, `test_prd_write_accepts_valid_single_archetype[system_evolution]`. |
